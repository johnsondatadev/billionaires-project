...
```{r FeatureSelection-Linear-R2CPBIC}
set.seed(123)
training <- .70
...
pcr_fitM <- pcr(finalWorth ~ ., data = lm_train, scale = TRUE, ncomp = 26)
```
...
Reaches highest R2 at 10 and plateaus.

```{r PLSTraining}
set.seed(123)

pls_fitM <- plsr(finalWorth ~ ., data = lm_train, scale = TRUE, ncomp = 10)
```
...
```{r PCREvaluation}
yHat_pcr <- predict(pcr_fitM, newdata = lm_test, ncomp = 26)
...
```{r PLSEvaluation}
yHat_pls <- predict(pls_fitM, newdata = lm_test, ncomp = 10)
