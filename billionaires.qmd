...
```{r ModelSelection_R2BICCIP}
which.max(full_base_model$adjr2)
```

```{r ModelSelection_Stepwise}
null_model <- lm(finalWorth ~ 1, data = Ttrain)
reg_step = step(full_base_model, scope = list(lower = null_model, upper = full_base_model), method = "backward") 
```
